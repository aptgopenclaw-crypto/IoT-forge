...
| **C** | 7 | RestClient timeout/retry/logging | ✅ | buildRestClient() + SimpleClientHttpRequestFactory |
| **C** | 8 | 真實 Nx Witness 環境整合測試 | ⏳ | 需有 Nx Witness Server 測試環境方可執行 |
| **C** | 9 | 端到端串流驗證（含 ZLMediaKit） | ⏳ | 需有完整 Docker Compose 環境方可執行 |
...
- [x] `NxWitnessAdapter.java` — `buildRestClient()` 加入：
...
- [ ] 加入 request/response 日誌攔截器（選用，後續可補）：
...
- [x] 單元測試層級不需測試 timeout（那是容器/環境設定）
- [ ] 可在 integration test 中驗證 timeout 行為（需 Step 8 環境）
...
- [ ] `NxWitnessAdapterIntegrationTest.java`（⏳ 需有 Nx Witness Server 測試環境）
...
- [ ] 準備一個 Nx Witness Server 測試環境（參見下方附錄）— ⏳ 待環境就緒
...
- [ ] `VmsE2eTest.java` — 完整端到端測試（需 `@DisabledIfEnvironmentVariable`，⏳ 待環境就緒）
...
| `vms/adapter/NxWitnessAdapter.java` | **修改** ✅ | 注入 NxSessionManager，認證改 session-based，healthCheck/listCameras/getCameraInfo 改用 REST API v1，mapStatus helper，RestClient timeout (5s/15s) |
...
- [x] `mvn clean verify` 無 compile error
- [x] `mvn spring-javaformat:apply -q` 無格式問題
