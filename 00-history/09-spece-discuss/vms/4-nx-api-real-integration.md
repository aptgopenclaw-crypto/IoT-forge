...
- [ ] `vms/adapter/NxSessionManager.java`
...
- [ ] `NxWitnessAdapterTest.java` — 主要變更：
...
- [ ] 若 `NxSessionManager.login()` 拋 `VMS_CONNECTION_FAILED("88103", 502)` — **已存在**
...
|---|---|---|---|
| `healthCheck()` | `GET /ec2/server/info` | `GET /rest/v1/system/info`（免授權） | ✅ 遷移 |
| `listCameras()` | `GET /ec2/cameras?page=&pageSize=` | `GET /rest/v1/devices` | ✅ 遷移 |
| `getCameraInfo()` | `GET /ec2/cameras/{id}` | `GET /rest/v1/devices/{id}` | ✅ 遷移 |
...
- [ ] `NxWitnessAdapter.java` — `healthCheck()` 改為：
...
- [ ] `NxWitnessAdapterTest.java` — `HealthCheck` 巢狀類別更新：
...
- [ ] 新增內部 record（取代現有 `NxCameraInfoResponse` / `NxCameraListResponse` / `NxCameraItem`）：
...
- [ ] `NxWitnessAdapter.java` — `listCameras()` 改為：
...
- [ ] `NxWitnessAdapter.java` — `getCameraInfo()` 改為：
...
- [ ] `NxWitnessAdapter.java` — 新增狀態映射輔助方法：
...
- [ ] `NxWitnessAdapter.java` — `getLiveStreamUrl()` / `getPlaybackUrl()` / `controlPtz()`
  - 維持 `/ec2/` 路徑不變
  - 更新 Javadoc 註明「REST API v1 無對應端點，使用保留的 `/ec2/` API；輸出經 ZLMediaKit 統一為 WebRTC」
...
- [ ] `ZlMediaKitClient.buildPlayUrl()` 回傳 `{publicUrl}/webrtcplayer/?streamId={id}&app=vms&schema=http`
- [ ] `VmsLiveView.vue` 以 `<iframe :src="playUrl">` 嵌入 ZLMediaKit webrtcplayer 頁面
- [ ] `VmsPlaybackView.vue` 同上
...
- [ ] 透過 `@DisabledIfEnvironmentVariable` 或 Maven profile 讓無環境時略過此測試
...
| `vms/adapter/NxWitnessAdapter.java` | **修改** | 注入 NxSessionManager，認證改 session-based，listCameras/healthCheck 改用 REST API v1 |
...
| 測試檔案 | 操作 | 說明 |
|---|---|---|
| `vms/adapter/NxSessionManagerTest.java` | **新增** | 6 案：login/cache hit/miss/invalidate/fail/error |
| `vms/adapter/NxWitnessAdapterTest.java` | **修改** | 注入 NxSessionManager mock，更新 healthCheck & listCameras URL 與回應格式 |
| `vms/adapter/NxWitnessAdapterIntegrationTest.java` | **新增** | `@SpringBootTest` 真實環境測試 |
...
- [ ] `mvn test -Dtest=NxSessionManagerTest` 全綠
- [ ] `mvn test -Dtest=NxWitnessAdapterTest` 全綠
- [ ] `mvn clean verify` 無 compile error
- [ ] `mvn spring-javaformat:apply -q` 無格式問題
- [ ] `get_errors` 無 vms 相關 error
- [ ] Integration test 在有 Nx Witness 環境下通過（可選）
