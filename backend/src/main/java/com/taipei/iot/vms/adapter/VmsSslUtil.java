 ...
 *
 * <p>
 * Nx Witness 等 VMS 伺服器通常使用自簽憑證，Java 預設 SSLContext 會拒絕連線。 此工具提供一個信任所有憑證的
 * {@link SSLContext}，僅用於內部 VMS 通訊（不涉及外部網路）。
 * </p>
 */
@Component
class VmsSslUtil {
	...
	static {
		try {
			...
			// 設定為 JVM 預設，使 SimpleClientHttpRequestFactory 等 HttpURLConnection-based
			// client 也能信任自簽憑證並跳過 hostname 驗證
			SSLContext.setDefault(INSECURE_SSL_CONTEXT);
			HttpsURLConnection.setDefaultHostnameVerifier(TRUST_ALL_HOSTNAME);
		}
		catch (Exception e) {
