...
class VmsWebhookControllerTest {
	...
	void nxWebhook_returns200() throws Exception {
		...
		mockMvc.perform(post("/v1/vms/webhook/NX_WITNESS").contentType(MediaType.APPLICATION_JSON).content(payload))
	...
	void unknownVmsType_returns200() throws Exception {
		mockMvc.perform(post("/v1/vms/webhook/UNKNOWN_VMS").contentType(MediaType.APPLICATION_JSON).content("{}"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.errorCode").value("00000"));
	}
