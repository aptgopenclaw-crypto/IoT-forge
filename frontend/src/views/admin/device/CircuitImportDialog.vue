...
import { importCircuits, downloadCircuitImportTemplate, downloadCircuitErrorReport } from '@/api/device'
...
async function handleImport() {
  ...
  state.value = 'loading'
  try {
    const res = await importCircuits(selectedFile.value)
    ...
    if (err?.response?.data?.body) {
      ...
      importResult.value = {
        ...
        totalRows: 0,
        successCount: 0,
        errors: [{ row: 0, field: '', value: '', message: err?.message || t('circuit.importFailed') }],
      }
    }
    state.value = 'error'
  }
...
async function handleDownloadTemplate(format: 'xlsx' | 'csv') {
  try {
    const res = await downloadCircuitImportTemplate(format)
    const blob = res as unknown as Blob
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `circuit-import-template.${format}`
    a.click()
...
async function handleDownloadErrorReport() {
  ...
  try {
    const blob = await downloadCircuitErrorReport({
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = 'circuit-import-error-report.csv'
    a.click()
    URL.revokeObjectURL(url)
  } catch {
    ...
    <template v-else-if="state === 'success' && importResult">
      <div class="import-result import-success">
        ...
        <p class="import-result-text">
          {{ t('circuit.importSuccessMessage', { count: importResult.successCount }) }}
        </p>
      </div>
    </template>
